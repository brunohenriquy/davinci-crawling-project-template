...
class GetAllTest(CaravaggioBaseTest):
    """ Test module for {{ project_name }}Resource model """
    resources = []
    ...
    def test_create_resources(self):
        for resource in self.resources:
            ...
            response = self.api_client.post(reverse("{{ project_name | lower }}-list"),
                                            data=json.dumps(
    ...
    def test_get_resources(self):
        for index, resource_id in enumerate(self.persisted_resources):
            ...
            path = "{0}{1}/".format(reverse("{{ project_name | lower }}-list"), resource_id)
    ...
    def test_search_text(self):
        ...
        """
        path = "{0}?text=distributed".format(reverse("{{ project_name | lower }}-search-list"))
        _logger.info("Path: {}".format(path))
        response = self.api_client.get(path)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_search_specialties(self):
        ...
        path = "{0}?specialties=Internet".\
            format(reverse("{{ project_name | lower }}-search-list"))
        _logger.info("Path: {}".format(path))
        response = self.api_client.get(path)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_search_geo(self):
        ...
        path = "{0}?km=10&from=44.59641,-123.25022".\
            format(reverse("{{ project_name | lower }}-geosearch-list"))
        _logger.info("Path: {}".format(path))
        response = self.api_client.get(path)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data["total"], 1)
